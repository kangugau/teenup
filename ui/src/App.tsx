...
import { Users, GraduationCap, Calendar, CreditCard, Plus } from "lucide-react";
import "./App.css";
...
function App() {
  ...
  const fetchData = async (day?: string) => {
    try {
      const [p, s, c, sub] = await Promise.all([
        ...
        axios.get(`${API_URL}/classes?day=${day ?? selectedDay}`),
  ...
  const onSelectedDayChange = (day: string) => {
    setSelectedDay(day);
    fetchData(day);
  };
    ...
    <div className="container">
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  await axios.post(`${API_URL}/classes`, {
                    ...classForm,
                    maxStudents: parseInt(classForm.maxStudents.toString()),
                  });
                  fetchData();
                }}
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setClassForm({ ...classForm, timeSlot: e.target.value })
                  }
                  required
                />
                <input
                  ...
                  onChange={(e) =>
                    setClassForm({ ...classForm, teacherName: e.target.value })
                  }
                  required
                />
                <input
      ...
      {activeTab === "users" && (
        <div className="grid">
          ...
          <div className="card">
            <h2>
              <GraduationCap size={20} /> Add Student
            </h2>
      ...
      {activeTab === "subscriptions" && (
        <div className="grid">
          <div className="card">
            <h2>
              <CreditCard size={20} /> New Subscription
            </h2>
            <form onSubmit={handleCreateSubscription}>
              <select
                value={subForm.studentId}
                onChange={(e) =>
                  setSubForm({ ...subForm, studentId: e.target.value })
                }
                required
              >
              ...
              <input
                ...
                onChange={(e) =>
                  setSubForm({ ...subForm, packageName: e.target.value })
                }
                required
              />
              <input
              ...
              <input
                ...
                onChange={(e) =>
                  setSubForm({ ...subForm, endDate: e.target.value })
                }
                required
              />
              <input
                  ...
                  <tr key={sub.id}>
                    ...
                    <td>
                      <button
                        onClick={async () => {
                          await axios.patch(
                            `${API_URL}/subscriptions/${sub.id}/use`,
                          );
                          fetchData();
                        }}
                        className="secondary"
